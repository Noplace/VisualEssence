...
Font::~Font() {
	std::map<int, CharacterDescription*>::iterator it = chars.begin();
	while( it != chars.end() )
	{
		delete it->second;
		it++;
...
int Font::Initialize(const char *fontFile, const char *fxFile, Context* context) {
  ...
  FILE* f = NULL;
	...
	//else
		loader = new BinaryLoader(f, this, fontFile);

	r = loader->Load();
	delete loader;

  vb.Initialize(context,chars.size()*4);
...
// Internal
CharacterDescription *Font::GetChar(int id)
{
	std::map<int, CharacterDescription*>::iterator it = chars.find(id);
...
{	
	CharacterDescription *ch = GetChar(first);
	if( ch == 0 ) return 0;
...
float Font::GetTextWidth(const char *text, int count)
{
	...
	for( int n = 0; n < count; )
	{
		...
		CharacterDescription *ch = GetChar(charId);
		if( ch == 0 ) ch = &defChar;
...
void Font::InternalWrite(float x, float y, float z, const char *text, int count, float spacing)
{
  ...
  //device->SetFVF(D3DFVF_FONTVERTEX);
	...
	for( int n = 0; n < count; )
	{
		...
		if( ch->page != page )
		{
			device->EndScene();
			page = ch->page;
			device->SetTexture(0,pages[page]);
      device->BeginScene();
		}

    static GameVertex vertex_data[4];
    memset(vertex_data,0,sizeof(vertex_data));
    vertex_data[0].color = 0xFFFFFFFF;
    vertex_data[1].color = 0xFFFFFFFF;
    vertex_data[2].color = 0xFFFFFFFF;
    vertex_data[3].color = 0xFFFFFFFF;

    vertex_data[0].x = x+ox;
    vertex_data[0].y = y-oy;
    vertex_data[0].z = z;
    vertex_data[0].u = u;
    vertex_data[0].v = v;

    vertex_data[1].x = x+w+ox;
    vertex_data[1].y = y-oy;
    vertex_data[1].z = z;
    vertex_data[1].u = u2;
    vertex_data[1].v = v;
    
    vertex_data[2].x = x+w+ox;
    vertex_data[2].y = y-h-oy;
    vertex_data[2].z = z;
    vertex_data[2].u = u2;
    vertex_data[2].v = v2;

    vertex_data[3].x = x+ox;
    vertex_data[3].y = y-h-oy;
    vertex_data[3].z = z;
    vertex_data[3].u = u;
    vertex_data[3].v = v2;

    device->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP,2,vertex_data,0); 
